...
def get_project_root() -> Path:
    ...
    while os.path.basename(current_file_path) != 'AlphaPressNet':
        current_file_path = os.path.dirname(current_file_path)
        if os.path.basename(current_file_path) == os.path.dirname(current_file_path):
            # We've reached the top-level directory without finding 'AlphaPressNet'
            raise ValueError("'AlphaPressNet' directory not found in the path hierarchy.")
